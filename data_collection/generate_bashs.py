...
for port in range(20000, 20008, 2):
...
def generate_script(ip, port, tm_port, route, scenario, carla_seed, traffic_seed, config_path):
    lines = []
    ...
    lines.append("export CHECKPOINT_ENDPOINT=${DATA_ROOT}/%s/results/%s.json\n" % (config_path.split(".")[0], route.split("/")[1].split(".")[0]))
    lines.append("\n")
...
for i in range(14):
    if not os.path.exists("bashs"):
        os.mkdir("bashs")
    if not os.path.exists(f"bashs/weather-{i}"):
        os.mkdir("bashs/weather-%d" % i)
    for route in routes:
        ip, port, tm_port = ip_ports[i % 4]
        ...
        fw = open("bashs/weather-%d/%s.sh" % (i, route.split("/")[1].split(".")[0]), "w")
        for line in script:
            fw.write(line)
